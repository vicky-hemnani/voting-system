...
def authentication(request):
    ...
    try:
        voter = Voters.objects.get(uuid=request.POST.get('aadhar_no'))
...
def verify_otp(request):
    ...
    if otp_input == request.session['otp']:
        print("VErified time", request.session['otp'])
...
def create_vote(request):
    ...
    if status[0]:
        try:
            ...
            context['status'] = 'We are not able to save your vote. Please try again. ' + \
                str(e)+'.'
